...
const char *ssid     = "NEFoods";
const char *password = "NEFoods@80";
...
bool connectToWifi() 
{
  ...
  digitalWrite(wLedPin, HIGH);               //Turn on the Wifi LED status indicator
  return true;
...
void updateDht()
{
  ...
  else
  {
    ...
    currentTemp[1] = (int)(newTemp*1.8)+32; 
    currentHumd = newHumd;
  }
  if (millis()>=nextCycle)
  {
    ...
    else if ((mmode == 1) && (currentTemp[u] < heatSetTo[u]))
      digitalWrite(heatRelayPin, HIGH);
    else if ((mmode == 2) && (currentTemp[u] > acSetTo[u]))
